        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <Card className="glass-card bg-green-400">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="join-code" className="text-green font-medium">Invitation Code</Label>
                  <Input id="join-code" placeholder="e.g., GRP-ABC123" value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} className="border-green/30 focus:border-green font-mono bg-green-500" />
                </div>
                <Button onClick={handleJoinByCode} className="w-full bg-gradient-green hover:bg-green text-white">
                  Join Group
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="glass-card bg-green-400">
              ...
              <CardContent>
                <div className="space-y-2">
                  <Label htmlFor="search" className="text-green font-medium">Search</Label>
                  <Input id="search" placeholder="Search by name, description..." value={searchQuery} onChange={e => handleSearch(e.target.value)} className="border-green/30 focus:border-green bg-green-500" />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="glass-card bg-green-400">
              <CardHeader>
                <CardTitle className="text-green text-sm">Public Groups</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-sm bg-green-400">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Total Available:</span>
                  <span className="font-medium text-green">24</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Most Popular:</span>
                  <span className="font-medium text-green">Office Lunch</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Categories:</span>
                  <span className="font-medium text-green">8</span>
                </div>
              </CardContent>
            </Card>
          </div>
              ...
              {filteredGroups.map(group => <Card key={group.id} className="glass-card hover-scale">
                  <CardContent className="p-6 bg-green-400">
                    <div className="flex items-start gap-4">
                      <Avatar className="w-16 h-16 border-4 border-green/20">
                      ...
                      <div className="flex-1 space-y-3">
                        <div className="flex items-start justify-between">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {group.privacy === "private" ? <Lock className="w-4 h-4 text-muted-foreground" /> : <Globe className="w-4 h-4 text-green" />}
                            </div>
                        ...
                        <div className="flex items-center justify-between pt-2">
                          <div className="flex items-center gap-2">
                            <Badge variant={group.privacy === "public" ? "default" : "secondary"} className={group.privacy === "public" ? "bg-green text-white" : ""}>
                              {group.privacy === "public" ? "Public" : "Private"}
                            </Badge>
                            {group.privacy === "public" && <Badge variant="outline" className="border-green-light text-green-light">
                                Instant Join
                              </Badge>}
                          </div>
                          <Button onClick={() => handleJoinGroup(group.name)} className="bg-gradient-green hover:bg-green text-white">
